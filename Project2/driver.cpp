...
class UI_PANEL
{
	...
	public:
		...
		AnalogSlider radiusSlider = AnalogSlider(36, 0.90f, 0 - 0.08f, 0.05f, 0.02f, 0 - 0.16f, 0 + 0.10f);
		Button fillInButton;
...
class UI_ElementSelector
{
	...
	public:
		...
		UI_ElementSelector()
		{
			...
			this->addSquare(this->wireFrame.vertices.size(), this->wireFrame.indices, 10);				// SQUARE <--------------
			// Copy vertices from expandButton's wireframe to wireFrame
...
int main()
{
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// If so, and is a left click; spawn smoke non-solid.
			if (userCursor._leftClick) {
				...
				if (!elementSlidingPanel.isExpanded()) {
					...
					if (userCursor.genID == 'K') {

						gaemWorld.entity_Batch.push_back(new Dynamic_Bombaclat(gaemWorld.CELL_GRID, userCursor.c, userCursor.r));
						userCursor._leftClick = false;
					}
					/*
					...
					//CELL_GRID[userCursor.r][userCursor.c] = new Dynamic_Bombaclat_Cell(oldOffset);

					entity_Batch.push_back(new Dynamic_Bombaclat(CELL_GRID, userCursor.c, userCursor.r));
				...
				elementSlidingPanel.checkPressed();
