		...
		void react()
		{
			...
			for (int i = 0; i < MAX_CELLS; ++i) { // For each row (down to up)
				for (int j = 0; j < MAX_CELLS; ++j) { // For each column (left to right)
					...
					CELL_GRID[i][j]->think(this->CELL_GRID, i, j);	// Each cell will think uniquely depending on definition (not abstract, still check empty cells [room for optimization]).
			...
			for (unsigned int i = 0; i < entity_Batch.size(); i++) {
				...
				switch(entity_Batch[i]->react(this->CELL_GRID)){
					case 'x':
						((Dynamic_Entity_Throwable_Bombaclat*&)entity_Batch[i])->shift(CELL_GRID, entity_Batch);
						break;
					case 'X':
						{
							...
							explodeAt(x, y, 25);
							delete this->entity_Batch[i];
							this->entity_Batch.erase((this->entity_Batch.begin()) + i);
							break;
						}
					case 'P':
						((Dynamic_Entity_Throwable_Rock*&)entity_Batch[i])->shift(CELL_GRID, entity_Batch);
						break;
					case 'B':
						((Dynamic_Entity_Throwable_Boulder*&)entity_Batch[i])->shift(CELL_GRID, entity_Batch);
						break;
					...
					if (dx * dx + dy * dy <= radiusSquared) {
						...
						if (x > -1 && x < MAX_CELLS && y > -1 && y < MAX_CELLS) {
								...
								if (rand() % 100 < 10) {
									...
									if (rand() % 10 == 1) {
										if (rand() % 20 == 1) {
											this->CELL_GRID[y][x] = new Dynamic_Molten_Shrapnel_Cell(oldOffset, dx, dy);
										}
										else {
